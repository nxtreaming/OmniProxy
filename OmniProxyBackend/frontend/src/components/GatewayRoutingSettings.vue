...
function routeConfig(route) {
  ...
  const current = props.config.gatewayRoutes[route.key]
  const model = current.model || route.fallback.model
  current.fallbacks.forEach((fallback) => {
...
function toggleFallbackProvider(route, provider) {
  ...
  current.fallbacks = [
    ...
    { provider, credentialType: '', model: current.model || route.fallback.model },
  ]
}
        ...
        <div class="gateway-route-controls">
          ...
          <div class="gateway-route-backend-field">
            ...
            <div v-if="fallbackConfigs(route).length" class="gateway-fallback-list">
              <div v-for="fallback in fallbackConfigs(route)" :key="fallback.provider" class="gateway-fallback-row">
