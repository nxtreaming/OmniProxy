...
func TestServiceModelRouteKeepsDeepSeek1MClientKeyAndNormalizesUpstream(t *testing.T) {
	var gotBody string
	...
	service, err := NewService(config.Config{
		...
		ModelRoutes: config.ModelRoutes{
			"deepseek-v4-pro[1m]": config.GatewayRouteConfig{
				Provider: token.ProviderDeepSeek,
	...
	if !strings.Contains(gotBody, `"model":"deepseek-v4-pro"`) || strings.Contains(gotBody, "deepseek-v4-pro[1m]") {
		t.Fatalf("expected model route request model to normalize for upstream, got %q", gotBody)
	}
}
