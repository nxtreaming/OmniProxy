...
- **Pi 单入口路由**：Pi Coding Agent 使用 `/pi-router/v1`，由 OmniProxy 按模型分流到对应上游。
- **TokenRouter provider**：新增 `tr_` API Key 管理、校验和 `/tokenrouter/v1` 本地路由。
- **OpenRouter 桌面体验**：支持模型列表刷新、余额查询和桌面端快速试聊。
...
| OpenAI / Codex | `auth.json` | 自动解析邮箱、access token、account id，刷新 Codex 订阅额度。 |
...
| --- | --- | --- |
| Stable | `v1.1.1` | 正式 Release，适合日常使用。 |
| Beta | `v1.1.1-beta.5` | Pre-release，适合验证新功能和回归修复。 |
