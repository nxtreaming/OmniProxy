...
| Forge AI | API Key | OpenAI Chat Completions 与 Anthropic Messages 双协议网关，默认 `https://forge-gateway-api.fly.dev/v1`。 |
...
| 客户端 | 支持内容 |
| --- | --- |
| Codex | 写入稳定 `/codex/v1` 网关入口，后端厂商、凭据和默认模型在网关路由中选择，支持恢复备份。 |
...
发布说明位于 `docs/releases/`。正式版发布前按 [发布检查清单](docs/release-checklist.md) 复核安装、更新、卸载和资产校验。Beta 版本会标记为 GitHub Pre-release，正式版保留给稳定发布。
