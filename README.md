...
OmniProxy 面向本地 AI 开发工作流设计。它可以让 Codex、Claude Code、OpenCode、Pi Coding Agent 以及兼容 OpenAI / Anthropic 协议的客户端先连接到本机代理，再由 OmniProxy 根据账号状态自动选择可用账号、记录用量、刷新额度、失败重试并切换账号。
...
- 🛠️ Codex / Claude Code / OpenCode / Pi Coding Agent 本地配置想自动写入，也希望能一键恢复。
